...
export default function FactoryReceiptOrdersPage() {
  ...
  const load = async () => {
    ...
    setRows(mergedRows.slice(from, from + pageSize));
    setLoading(false);
  };
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b border-slate-100 text-slate-700">
                ...
                <th className="p-4 text-left text-[14px] font-bold uppercase tracking-widest">ເບີລູກຄ້າ</th>
                ...
                <th className="p-4 text-center text-[14px] font-bold uppercase tracking-widest">WhatsApp</th>
              ...
              {!loading && rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={`${row.receiptId}-${row.orderId}`} className="transition-colors hover:bg-slate-50/80">
                    ...
                    <td className="p-4 font-semibold text-slate-700">{row.customerPhone?.trim() ? row.customerPhone : row.customerWhatsapp?.trim() ? row.customerWhatsapp : "-"}</td>
